...
const AuthModal = ({ isOpen, onClose, onLogin, guestData, initialMode = 'signin' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = view === 'signup' ? { ...formData, guestData } : { email: formData.email, password: formData.password };
    ...
    try {
      ...
      if (view === 'forgot') {
        setMessage({ text: 'Link sent!', type: 'success' });
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-slate-900 border border-slate-700 w-full max-w-md p-8 rounded-2xl shadow-2xl animate-fade-in-up">
        ...
        {message.text && <div className={`p-3 rounded-xl mb-4 text-center text-xs font-bold border ${message.type === 'error' ? 'bg-red-500/10 border-red-500/50 text-red-400' : 'bg-emerald-500/10 border-emerald-500 text-emerald-400'}`}>{message.text}</div>}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          {view === 'signup' && (
            <div className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 flex items-center">
              ...
              <input type="text" placeholder="Username" className="bg-transparent text-white w-full outline-none font-medium" value={formData.username} onChange={(e) => setFormData({...formData, username: e.target.value})} required />
            </div>
          )}
          <div className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 flex items-center">
            <Mail size={18} className="text-slate-500 mr-3" />
            <input type="email" placeholder="Email" className="bg-transparent text-white w-full outline-none font-medium" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required />
          </div>
          {view !== 'forgot' && (
            <div className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 flex items-center">
              <Lock size={18} className="text-slate-500 mr-3" />
              <input type="password" placeholder="Password" className="bg-transparent text-white w-full outline-none font-medium" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} required />
            </div>
          )}

          <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl shadow-lg mt-2 uppercase italic tracking-wider flex justify-center gap-2 transition-transform active:scale-95">
